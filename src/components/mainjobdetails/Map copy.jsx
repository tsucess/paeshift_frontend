  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h3 style={{ marginBottom: '15px' }}>Confirm Location Update</h3>
        <p>Are you sure you want to update the job location? This action cannot be undone.</p>
        <div style={{
          display: 'flex',
  ...
  const saveLocationToBackend = async (newPosition, address) => {
    try {
      const response = await Axios.post(
        `${import.meta.env.VITE_API_BASE_URL || 'http://127.0.0.1:8000'}/jobs/${jobId}/location/update/`,
        {
  ...
  const handleApiLoaded = ({ map, maps }) => {
    ...
    if (jobDetails.latitude && jobDetails.longitude) {
      const jobMarker = new maps.Marker({
        ...
        map: map,
        draggable: profile?.role === 'client', // Only allow dragging if user is a client
        title: "Job Location"
      });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await Axios.get(`${import.meta.env.VITE_API_BASE_URL || 'http://127.0.0.1:8000'}/jobs/${jobIdToUse}`);
      ...
      {jobDetails.latitude && jobDetails.longitude && userLocation.lat && userLocation.lng ? (
        <div className="col-12 m-0 p-0" style={{ height: '500px', width: '100%', position: 'relative' }}>
          ...
          <ConfirmModal
            ...
            onConfirm={handleLocationUpdateConfirm}
          />
        </div>
      ) : (
