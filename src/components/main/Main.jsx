...
import { faServer, faUser, faUserGroup, faSearch, faMapPin, faLocation, faLocationDot, faChevronDown, faCheck } from "@fortawesome/free-solid-svg-icons";
...
const Main = () => {
  ...
  // const [allJobs, setAllJobs] = useState([]); // should be an array, not string
  ...
  const { data: jobsData = { jobs: [], pagination: {} }, isLoading: loadingJobs, error: errorJobs, refetch: refetchJobs } = useQuery({
    queryKey: ['allJobs', currentUserId],
  ...
  useEffect(() => {
    ...
    if (currentUserId) {
      ...
      console.warn('No user_id found in localStorage. Skipping account details request.');
    }
  ...
  // Filtering logic before rendering (memoized for performance)
  ...
  }, [allJobs, filters.type, filters.industry, searchWork]);

  // Reset page to 1 when filters or search changes
  useEffect(() => {
    setCurrentPage(1);
  }, [filters.type, filters.industry, searchWork]);
          ...
          <div className="cards">
            {!profile.phone_number || !profile.location ? (
              <div className="text-center w-100 p-5">
                <p>
                  Update your Phone number and Address in settings — click to{" "}
                  <Link to="/settings" className="fw-bold text-purple">
                    Update Profile
                  </Link>
                </p>
              </div>
                ...
                return (
                  <article className="card" key={item.id}>
                    <div className="card_top">
                      <span className="profile_info">
                        ...
                        <span>
                          ...
                          <span className="rate_score">{item.client_rating}</span>
                        </span>
                      </span>
                      <span className="top_cta">
                        {item.has_applied ? (
                          ...
                          >
                            ...
                            {/* <FontAwesomeIcon icon={faCheck} className="icon-applied" /> */}
                          </button>
                        ) : (
                          <button
                            className={isJobSaved ? "btn active" : "btn"}
                            onClick={() => saveJob(profile.user_id, item.id)}
                          >
                    ...
                    <div className="price">
                      <span>
                        <h6>
                          ₦ {Math.ceil(item.rate / item.applicants_needed)} /hr
                        </h6>
                        <p>{item.applicants_needed} applicant needed</p>
                      </span>
                      <span>
                        <Link to={`../jobdetails/${item.id}`} className="btn">
                          View Job Details
                        </Link>
                      </span>
                    </div>
